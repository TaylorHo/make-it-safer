...
async function auditFix() {
	...
	const runAuditScan = async (type) => {
		try {
			return JSON.parse(
				await cmd(`npm audit ${type} --no-progress --audit-level=none --json`),
			);
		} catch (error) {
			hasError.push(error);
			console.error(
				"❌ Error on executing audit fix:",
				error.summary?.split(EOL)[0] ||
					error?.summary ||
					error?.message ||
					error,
			);
	...
	if (hasError.length > 0) {
		...
		showFixed(
			auditResultJSON.vulnerabilities,
			auditRemanescentJSON.audit.vulnerabilities,
		);
	}
}
