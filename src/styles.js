...
const showUpdated = (packages) => {
	...
	for (const currentPackage of formattedPackages) {
		const {
			packageName,
...
const showFixed = (previousVulns, afterVulns) => {
	...
	for (const vulnerablePackage of Object.keys(previousVulns)) {
		...
		if (
			previousVulns[vulnerablePackage].severity.length > largestSeverityName
		) {
	...
	for (const vulnerablePackage of Object.keys(previousVulns)) {
		if (previousVulns[vulnerablePackage].fixAvailable === true) {
			...
			const vulnerabilitySeverity = getSeverity(
				previousVulns[vulnerablePackage].severity,
				largestSeverityName,
			);
	...
	for (const vulnerablePackage of Object.keys(afterVulns)) {
		if (typeof afterVulns[vulnerablePackage].fixAvailable === "object") {
			const vulnerablePackageName = afterVulns[vulnerablePackage].name;
			const vulnerabilitySeverity = getSeverity(
				afterVulns[vulnerablePackage].severity,
				largestSeverityName,
			);
			const fixAvailablePackage =
				afterVulns[vulnerablePackage].fixAvailable.name;
